...
router = APIRouter()
from schema.activity_admin import (
    PendingActivitiesResponse,
    PendingActivityItem,
    AdminActivityUpdateRequest,
    AdminActivityUpdateResponse,
)
...
async def get_pending_activities(
    request: Request, user_id: str = Query(...), token: str = Query(...)
):
...
async def admin_update_activity(body: AdminActivityUpdateRequest, request: Request):
    session = get_session(request)
